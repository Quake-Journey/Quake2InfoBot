...
function parseLine(line) {
  ...
  for (let i = 0; i < line.length; i++) {
    ...
    if (char === '"') {
      inQuotes = !inQuotes;
      if (!inQuotes) { // Добавляем current только при закрывающей кавычке
        if (current) { 
          parts.push(current); 
          current = ''; 
        }
  ...
  return {
    ...
    score: parts[1], 
    class: parseInt(parts[2], 10),
    ping: parseInt(parts[3], 10),
    name: parts[4],
    weapon: parts[5] || '',
    unknown1: parseInt(parts[6], 10),
    unknown2: parseInt(parts[7], 10),
    unknown3: parts[8] || ''
  };
}
...
function parseLine2222(line) {
  ...
  return {
    score: parseInt(parsedLine[0], 10), //  исправлено:  id  -  первый  элемент
    team: parseInt(parsedLine[1], 10),
    class: parseInt(parsedLine[2], 10),
    health: parseInt(parsedLine[3], 10),
    name: parsedLine[4],
    weapon: parsedLine[5] || '', // Используем parts[6] или пустую строку, если parts[6] не определен
    unknown1: parseInt(parsedLine[6], 10),  //  исправлено:  unknown1  -  7-й  элемент
    unknown2: parseInt(parsedLine[7], 10),  //  исправлено:  unknown2  -  8-й  элемент
    unknown3: parsedLine[8] || '' // Используем parts[9] или пустую строку, если parts[9] не определен
  };
}
